...
modify_activity_data <- function(activity_df) {
  ...
  activity_df$Reduced_Application[activity_df$Reduced_Application==""]<-"NA"
    ...
    for (day in day_list) {
      ...
      for (treatment in unique(day_df$Treatment)) {
        ...
        if (treatment=='WS') {
          ...
          app_colors <- gg_color_hue(length(unique(activity_df$Reduced_Application)))
          app_colors[unique(activity_df$Reduced_Application)=="NA"] <- "white"

          app_usage_plot <- ggplot(activity_df, aes(x=TreatmentTime)) +
            geom_point(aes(y=factor(1), colour=Reduced_Application), shape=15, size=5) +
            scale_color_manual(values=app_colors, limits = unique(activity_df$Reduced_Application)) +
            xlab('Time [s]')
