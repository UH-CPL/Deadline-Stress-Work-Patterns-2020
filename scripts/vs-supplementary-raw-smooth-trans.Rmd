...
draw_signal_plot_for_paper <- function(test=F) {
  subj_list <- c('T017')
  day_list <- c('Day2')
  signal_list <<- c('PP')

  for (subj in subj_list) {
    subj_df <- custom_read_csv(file.path(project_dir, curated_data_dir, physiological_data_dir, 'T017_Day2.csv')) %>% filter(Participant_ID==subj)
    
    for (day in day_list) {
      day_df <- subj_df %>% filter(Day==day) 
      
      # convert_to_csv(day_df, file.path(project_dir, curated_data_dir, physiological_data_dir, 'T017_Day2.csv'))
