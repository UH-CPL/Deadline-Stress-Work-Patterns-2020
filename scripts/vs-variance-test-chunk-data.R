...
#---FUNCTION DEFINITION---#
#-------------------------#
read_data <- function(chunk_size_minute) {
...
variance_test <- function(df, chunk_size_minute) {
  # print(df)
  # print(nrow(df))
  # print(head(df, 2))
...
process_variance_test <- function(chunk_size_minute) {
  variance_test_df <<- tibble()
  ...
  mean_df %>%
    filter(Mask==1,
           Total_Non_NA_Rows>chunk_size_minute*60*percentage/100
           ) %>%
    group_by(Participant_ID) %>%
    do(variance_test(., chunk_size_minute))
}

conduct_variance_tests_chunk_data <- function() {
  print(chunk_sizes)
  for (chunk_size_minute in chunk_sizes) {
    read_data(chunk_size_minute)
    process_variance_test(chunk_size_minute)
  }
}
