...
merge_data <- function() {
  ...
  qc0_df <- custom_read_csv(file.path(physiological_data_path, qc0_final_file_name)) %>% 
    select(Participant_ID, Day, Treatment, TreatmentTime, Raw_PP, Raw_E4_EDA, Raw_E4_HR, Raw_iWatch_HR)
  ...
  full_df <- transformed_df %>%   
    merge(qc0_df, by=c('Participant_ID', 'Day', 'Treatment', 'TreatmentTime'), all.x=T) %>% 
    merge(qc_df, by=c('Participant_ID', 'Day', 'Treatment', 'TreatmentTime'), all.x=T) %>%
