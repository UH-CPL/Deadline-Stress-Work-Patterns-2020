...
mat <- melt(mytable_mat,na.rm = TRUE)
  ...
  #print(mat)
    ...
    scale_fill_gradientn(colours = c("white", "yellow", "red"), name = "", limits=c(0,100))+
    new_scale("fill") +
...
for (i in 1:n) {
  ...
  if(str_detect(title, "T003")){
    ...
    #geom_tile(aes(fill=value2))+
    geom_tile(aes(fill=percent_in_num))+
    geom_text(aes(label=percent_in_num), show.legend = FALSE,col="black",size=3, face = "bold")+
    scale_fill_gradientn(colours = c("lightgray","dimgray"), name = "Time [s] %", limits=c(0,100))+
    new_scale("fill") +
    geom_tile(aes(fill=percent_in_num), data = subset(mat, value1 > -1))+
    geom_text(aes(label=percent_in_num), show.legend = FALSE, col="black", size=3, data = subset(mat, value1 > 0))+
...
#ggarrange(plotlist = plot_list_heatmap, nrow = 2, ncol = 2, align = c("hv"), common.legend = TRUE, legend="right")

heatmap_list=list()
heatmap_list=ggarrange(plotlist = plot_list_heatmap, nrow = 2, ncol = 2, align = c("hv"), common.legend = TRUE, legend="right")
heatmap_list
...
Draw_Timeline<-function(Data_Frame, Title, Color){
  ...
  theme(legend.position = "bottom")+theme(legend.title = element_blank(), legend.text=element_text(size=10), legend.background = element_blank())+
...
pal <- c( "CR" = "green4",  "PR" = "lightgreen", "CW" = "blue", "PW" = "cyan", "PI" = "black","VI" = "yellow","Out" = "white","SP" = "red","ELD" = "orange","EiP" = "magenta", "T" = "brown", "nil"="gray92")

DF_Sub<-subset(DF, Participant_ID=="T013" & Treatment=="WS" & Day=="Day4")
#write.csv(DF_Sub, 'T009_Day2.csv')
      # tab <- table(DF_Sub$Activities_QC1)
      # DF_Sub<-DF_Sub[DF_Sub$Activities_QC1 %in% names(tab)[tab>60],]
      
      DF_Sub$B[DF_Sub$Ontology_Two==""]<-"nil"
      DF_Sub$C[DF_Sub$Ontology_Three==""]<-"nil"
      
timeline<-ggplot(DF_Sub,aes(x=Sinterface_Time, y=Day)) +
  geom_point(data = DF_Sub[which(DF_Sub$Day=="Day4"),], aes(y=factor(1), colour=Ontology_One), shape=15, size=10)+
  geom_point(data = DF_Sub[which(DF_Sub$Day=="Day4"),], aes(y=1.12, colour=Ontology_Two), shape=15, size=10)+
  geom_point(data = DF_Sub[which(DF_Sub$Day=="Day4"),], aes(y=1.24, colour=Ontology_Three), shape=15, size=10)+
  scale_y_discrete(breaks=c("1", "2", "3", "4"),labels=c("Day1", "Day2","Day3", "Day4"))+
...
plot_location<-"C:/Users/soton/Documents/R/DeadlineProject/Plots/Combined/Version2/"
...
Draw_Timeline_Heatmap<-function(Data_Frame, Title){
  Timeline=Draw_Timeline_Day(Data_Frame, Title)
  #print(Timeline)
...
Draw_Timeline_Day<-function(Data_Frame, Title){
      tab_Timeline <- table(Data_Frame$Activities_QC1)
      Data_Frame<-Data_Frame[Data_Frame$Activities_QC1 %in% names(tab_Timeline)[tab_Timeline>60],]
      Data_Frame$Ontology_Two[Data_Frame$Ontology_Two==""]<-"nil"
      Data_Frame$Ontology_Three[Data_Frame$Ontology_Three==""]<-"nil"
      ...
      if (Title!="Day4"){
        timeline<-ggplot(Data_Frame,aes(x=Sinterface_Time, y=Day)) +
        geom_point(aes(y=factor(1), colour=Ontology_One), shape=15, size=10)+
        geom_point(aes(y=1.12, colour=Ontology_Two), shape=15, size=10)+
        geom_point(aes(y=1.24, colour=Ontology_Three), shape=15, size=10)+
        ...
        scale_color_manual(values = Ontology_Color, limits = c("CR", "PR", "CW", "PW", "PI", "VI", "Out", "SP", "Out", "ELD", "EiP", "T"))+
...
Draw_HeatMap_Day<-function(DF_Sub_Day, Title){
    tab_heatmap <- table(DF_Sub_Day$Activities_QC1)
    DF_Sub_Day<-DF_Sub_Day[DF_Sub_Day$Activities_QC1 %in% names(tab_heatmap)[tab_heatmap>60],]
    title=paste(levels(sid)[i],levels(D_id)[j])
  ...
  if(str_detect(title, "T003")){
    ...
    #scale_fill_gradientn(colours = c("lightgray","dimgray"), name = "Time [s] %", limits=c(0,100))+
    scale_fill_gradientn(colours = c("gray","yellow", "red"), name = "Time [s] %", limits=c(0,100))+
    new_scale("fill") +
    geom_tile(aes(fill=percent_in_num), data = subset(mat, value1 > -1))+
