...
read_data <- function() {
  ...
  raw_mean_df <- custom_read_csv(file.path(physiological_data_path, qc1_lm_mean_v1_file_name)) %>% 
    rename(PP_Raw=PP,
           E4_HR_Raw=E4_HR,
           E4_EDA_Raw=E4_EDA,
           iWatch_HR_Raw=iWatch_HR)
  
  raw_rb_mean_df <<- raw_mean_df %>% 
    filter(Treatment=='RB')
