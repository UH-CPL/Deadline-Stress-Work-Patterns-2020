...
multi_level_segment_df_file_name <- 'multi_level_segment_df.csv'
...
generate_daywise_mean_data <- function(mean_df, output_v2_file_name) {
  daywise_mean_df <- mean_df %>%
    ...
    mutate(Day3_Day4_Min = pmin(Day3, Day4, na.rm = T),
           Four_Day_Min = pmin(Day1, Day2, Day3, Day4, na.rm = T),
           # Four_Day_Mean = mean(Day1, Day2, Day3, Day4, na.rm = T),
           Four_Day_Mean = rowMeans(cbind(Day1, Day2, Day3, Day4), na.rm=T))
